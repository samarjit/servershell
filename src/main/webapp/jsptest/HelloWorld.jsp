...
<script>
	...
	<s:if test = "jsrule != null" >
		 rulesframework =  ${jsrule};
        	...
        	var fieldlist = "name, email".split(",");
  ...
  function calljqgrid(formdata){
   jQuery("#listid").jqGrid( {
      	...
      	url:'<%= request.getContextPath() %>/jqgrid.action?command=true&screenName=HelloWorld&submitdata={bulkcmd="gridonload"}',
      	datatype: "json",
      	height:350, 
      	colNames:['Name','Email'      	],
      	colModel:[
      	{name: 'name', index: 'name' , width:360, editable:true },
      	{name: 'email', index: 'email' , width:360, editable:true }
      	],
      	rowNum: 15,
      	rowList: [ 15, 25, 50],
      	pager: '#pagerid',
       ...
       onSelectRow: function(id){
	    ...
	    loadComplete: function(){
			       ...
			       editurl: "${pageContext.request.contextPath}/html/simpleform.action?screenName=HelloWorld&bulkcmd=grid",
       caption: "HelloWorld"
  ...
  function ajaxSubmit(){
	jQuery.post("${pageContext.request.contextPath}/html/simpleform.action?screenName=HelloWorld", 
			$("#form1").serialize(),
			function(data){
...
<!--Submit Form -->
<form name="form1" id="form1" method="post" action="${pageContext.request.contextPath}/html/simpleform.action?screenName=HelloWorld">
        	 <table>
